...
function createIndexPanel() {
    // index panel
    const indexPanel = document.querySelector("body").appendChild(document.createElement("div"));
    indexPanel.setAttribute("id", "ivan_search-engine-switch");
    indexPanel.className = "fixed left-4 top-1/3 -translate-y-1/2 bg-sky-100/50 backdrop-blur text-sky-700 rounded shadow-lg flex flex-col";
    ...
    foldBtn.className = 'absolute -right-2 top-1/2 -translate-y-1/2 h-6 w-6 rounded-full shadow-lg bg-sky-700/50 backdrop-blur text-sky-100 flex items-center justify-center text-xl';
...
function registerJump() {
    let linkElems = document.querySelectorAll(".switch-search-engine");
    for (const elem of linkElems) {
        elem.addEventListener("click", function () {
            const currUrl = new URL(window.location.href);
            const currSearchEngine = searchEngines.find(it => it.regUrl?.test(currUrl.toString()));

            if (!currSearchEngine) {
                GM_log(`The current page does not match the preset search engine. url: ${currUrl}`);
                return
            }
